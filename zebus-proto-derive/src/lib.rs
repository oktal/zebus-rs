...
fn try_message(input: TokenStream) -> Result<TokenStream, Error> {
    ...
    let expanded = quote! {
        impl #impl_generics ::zebus_proto::Message for #ident #ty_generics #where_clause {
            #[allow(unused_variables)]
            fn encode_raw(&self, buf: &mut dyn ::zebus_proto::bytes::BufMut) {
                #(#encode)*
            }
            ...
            fn merge_field<B>(
                &mut self,
                tag: u32,
                wire_type: ::zebus_proto::encoding::WireType,
                buf: &mut B,
                ctx: ::zebus_proto::encoding::DecodeContext,
            ) -> ::core::result::Result<(), ::zebus_proto::DecodeError>
            where B: ::zebus_proto::bytes::Buf {
                #struct_name
                match tag {
                    #(#merge)*
                    _ => ::zebus_proto::encoding::skip_field(wire_type, tag, buf, ctx),
                }
            }
...
fn try_oneof(input: TokenStream) -> Result<TokenStream, Error> {
    ...
    let expanded = quote! {
        impl #impl_generics #ident #ty_generics #where_clause {
            ...
            pub fn encode<B>(&self, buf: &mut B) where B: ::zebus_proto::bytes::BufMut {
                match *self {
                    #(#encode,)*
                }
            }
