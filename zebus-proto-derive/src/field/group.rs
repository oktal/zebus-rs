...
impl Field {
        ...
        match self.label {
            Label::Optional => quote! {
                if let Some(ref msg) = #ident {
                    ::zebus_proto::encoding::group::encode(#tag, msg, buf);
                }
            },
            Label::Required => quote! {
                ::zebus_proto::encoding::group::encode(#tag, &#ident, buf);
            },
    ...
    pub fn merge(&self, ident: TokenStream) -> TokenStream {
        match self.label {
            Label::Optional => quote! {
                ::zebus_proto::encoding::group::merge(
                    tag,
                    wire_type,
            ...
            Label::Required => quote! {
                ::zebus_proto::encoding::group::merge(tag, wire_type, #ident, buf, ctx)
            },
            Label::Repeated => quote! {
                ::zebus_proto::encoding::group::merge_repeated(tag, wire_type, #ident, buf, ctx)
            },
        }
    }
    ...
    pub fn encoded_len(&self, ident: TokenStream) -> TokenStream {
        ...
        match self.label {
            Label::Optional => quote! {
                #ident.as_ref().map_or(0, |msg| ::zebus_proto::encoding::group::encoded_len(#tag, msg))
            },
            Label::Required => quote! {
                ::zebus_proto::encoding::group::encoded_len(#tag, &#ident)
            },
            Label::Repeated => quote! {
                ::zebus_proto::encoding::group::encoded_len_repeated(#tag, &#ident)
            },
        }
    }
