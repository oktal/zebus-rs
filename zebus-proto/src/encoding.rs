    ...
    ($ty:ty,
     ...
     from_uint64($from_uint64_value:ident) $from_uint64:expr) => (
         ...
         pub mod $proto_ty {
            ...
            pub fn encode(tag: u32, $to_uint64_value: &$ty, buf: &mut dyn BufMut) {
                encode_key(tag, WireType::Varint, buf);
                encode_varint($to_uint64, buf);
            }

            pub fn merge<B: Buf>(wire_type: WireType, value: &mut $ty, buf: &mut B, _ctx: DecodeContext) -> Result<(), DecodeError> {
...
pub mod bytes {
    ...
    pub fn encode<A>(tag: u32, value: &A, buf: &mut dyn BufMut)
    where
        A: BytesAdapter,
    {
        encode_key(tag, WireType::LengthDelimited, buf);
...
pub mod message {
    ...
    pub fn encode<M>(tag: u32, msg: &M, buf: &mut dyn BufMut)
    where
        M: Message,
    {
        encode_key(tag, WireType::LengthDelimited, buf);
...
macro_rules! map {
    ($map_ty:ident) => {
        ...
        pub fn encode<K, V, KE, KL, VE, VL>(
            ...
            V: Default + PartialEq,
            KE: Fn(u32, &K, &mut dyn BufMut),
            KL: Fn(u32, &K) -> usize,
            VE: Fn(u32, &V, &mut dyn BufMut),
            VL: Fn(u32, &V) -> usize,
        {
            encode_with_default(
                key_encode,
...
mod test {
    ...
    pub fn check_type<T, B>(
        ...
        tag: u32,
        wire_type: WireType,
        encode: fn(u32, &B, &mut dyn BufMut),
